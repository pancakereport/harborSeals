...
C.model=matrix(              list("BL_A",0,0,0,0,"MOCI_A",
                                  ...
                                  nrow = 15, ncol = 6,
                                  byrow = TRUE)
m.5pop_Coyote_MOCI=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                            C = C.model,
                                            c = small_c_Coyote_01_MOCI),
                                            control=list(maxit=3000, safe=FALSE, trace = 0)) 

df_aic <- df_aic %>% add_row(model = "H01 - m.Ind_Coyote_MOCI", aic = m.5pop_Coyote_MOCI$AIC)
...
m.3pop_Coyote_MOCI=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                       C = C.model,
                                       c = small_c_Coyote_01_MOCI),
                       control=list(maxit=3000, safe=TRUE, trace = 1)) 

df_aic <- df_aic %>% add_row(model = "H02 - m.3pop_Coyote_MOCI", aic = m.3pop_Coyote_MOCI$AIC)
df_aic
...
C.model=matrix(list(0,0,0,0,0,"MOCI_ADULT",
               ...
               nrow = 3, ncol = 6,
               byrow = TRUE)
m.3Class_MOCI=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                       C = C.model,
                                       c = small_c_Coyote_01_MOCI),
                       control=list(maxit=3000, safe=TRUE, trace = 0)) 

df_aic <- df_aic %>% add_row(model = "H03 - m.3Class_MOCI", aic = m.3Class_MOCI$AIC)
df_aic
...
m.MOLT_IND_MOCI=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                                  C = C.model,
                                                  c = small_c_Coyote_01_MOCI),
                                  
                                  control=list(maxit=5000, safe=TRUE, trace = 1)) 

df_aic <- df_aic %>% add_row(model = "H04 - m.MOLT_IND_MOCI", aic = m.MOLT_IND_MOCI$AIC)
df_aic
...
C.model=matrix(list("BL_B",0,0,0,0, "MOCI_B", #1
               ...
               nrow = 10, ncol = 6,
               byrow = TRUE)
m.MOLT_IND_5SITE_COYOTE_MOCI=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                   C = C.model,
                                   c = small_c_Coyote_01_MOCI),
                   control=list(maxit=5000, safe=TRUE)) 

df_aic <- df_aic %>% add_row(model = "H05A - m.MOLT_IND_5SITE_COYOTE_MOCI", aic = m.MOLT_IND_5SITE_COYOTE_MOCI$AIC)
df_aic
...
C.model=matrix(list("BL_B",0,0,0,0, "MOCI_B", #1
               ...
               nrow = 10, ncol = 6,
               byrow = TRUE)
m.MOLT_IND_SITE_MOCI_Q_unc=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                           C = C.model,
                                           c = small_c_Coyote_01_MOCI),
                           control=list(maxit=5000, safe=TRUE)) 

CIs <- MARSSparamCIs(m.MOLT_IND_SITE_MOCI_Q_unc, alpha = 0.11)
CIs

beepr::beep(0)
m.MOLT_IND_SITE_MOCI_Q_unc$AIC 

df_aic <- df_aic %>% add_row(model = "H05B - m.MOLT_IND_SITE_MOCI_Q_unc", aic = m.MOLT_IND_SITE_MOCI_Q_unc$AIC)
df_aic
...
C.model=matrix(list("BL_B",0,0,0,0, "MOCI_B",#1
               ...
               nrow = 10, ncol = 6,
               byrow = TRUE)
m.MOLT_IND_SITE_MOCI.equal_B_diag_uneq=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                                 C = C.model,
                                                 c = small_c_Coyote_01_MOCI),
                                 control=list(maxit=5000, safe=TRUE)) 

df_aic <- df_aic %>% add_row(model = "H05C - m.MOLT_IND_SITE_MOCI.equal_B_diag_uneq", aic = m.MOLT_IND_SITE_MOCI.equal_B_diag_uneq$AIC)
df_aic
...
C.model=matrix(list("BL_B",0,0,0,0, "MOCI_B", #1
               ...
               nrow = 10, ncol = 6,
               byrow = TRUE)
m.MOLT_IND_SITE_Coy_rate_MOCI.equal_B_diag_uneq=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                                             C = C.model,
                                                             c = small_c_Coyote_rate_MOCI),
                                             control=list(maxit=5000, safe=TRUE)) 

df_aic <- df_aic %>% add_row(model = "H05H - m.MOLT_IND_SITE_Coy_rate_MOCI.equal_B_diag_uneq", aic = m.MOLT_IND_SITE_Coy_rate_MOCI.equal_B_diag_uneq$AIC)
df_aic
...
C.model=matrix(list("BL_ADULT",0,0,0,0, "MOCI_ADULT", #1
               ...
               nrow = 10, ncol = 6,
               byrow = TRUE)
m.PUP_IND_SITE_MOCI=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                           C = C.model,
                                           c = small_c_Coyote_01_MOCI),
                           control=list(maxit=5000, safe=TRUE)) 

df_aic <- df_aic %>% add_row(model = "H06 - m.PUP_IND_SITE_MOCI", aic = m.PUP_IND_SITE_MOCI$AIC)
df_aic
...
C.model=matrix(list(0,0,0,0,0, "MOCI", #1
               ...
               nrow = 3, ncol = 6,
               byrow = TRUE)
m.BL_SOUTH_NORTH_MOCI=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                          C = C.model,
                                          c = small_c_Coyote_01_MOCI),
                          control=list(maxit=5000, safe=TRUE)) 

df_aic <- df_aic %>% add_row(model = "H08 - m.BL_SOUTH_NORTH_MOCI", aic = m.BL_SOUTH_NORTH_MOCI$AIC)
df_aic
...
C.model=matrix(list("BL_ADULT",0,0,0,0, "MOCI_ADULT", #1
                    ...
                    nrow = 6, ncol = 6,
                    byrow = TRUE)
m.BL_SOUTH_NORTH_PUP_IND_MOCI=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                            C = C.model,
                                            c = small_c_Coyote_01_MOCI),
                            control=list(maxit=5000, safe=TRUE)) 

df_aic <- df_aic %>% add_row(model = "H09 - m.BL_SOUTH_NORTH_PUP_IND_MOCI", aic = m.BL_SOUTH_NORTH_PUP_IND_MOCI$AIC)
df_aic
...
C.model=matrix(list("BL_ADULT",0,0,0,0, "MOCI_Breed", #1
               ...
               nrow = 6, ncol = 6,
               byrow = TRUE)
m.BL_SOUTH_NORTH_MOLT_IND_MOCI=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                                    C = C.model,
                                                    c = small_c_Coyote_01_MOCI),
                                    control=list(maxit=5000, safe=TRUE)) 

df_aic <- df_aic %>% add_row(model = "H10 - m.BL_SOUTH_NORTH_MOLT_IND_MOCI", aic = m.BL_SOUTH_NORTH_MOLT_IND_MOCI$AIC)
df_aic
...
C.model=matrix(list("BL_ADULT",0,0,0,0, "MOCI_Breed", #1
               ...
               nrow = 4, ncol = 6,
               byrow = TRUE)
m.2POP_MOLT_IND_MOCI=MARSS(dat, model=list(Z=Z.model, U=U.model, Q=Q.model, R=R.model, B=B.model,
                                                     C = C.model,
                                                     c = small_c_Coyote_01_MOCI),
                                     control=list(maxit=5000, safe=TRUE)) 

df_aic <- df_aic %>% add_row(model = "H11 - m.2POP_MOLT_IND_MOCI", aic = m.2POP_MOLT_IND_MOCI$AIC)
df_aic
...
d2 <- d %>% pivot_longer(cols = c(2:11), names_to = "Subsite_Season", values_to = "log_est")
d2.se <- d.se %>% pivot_longer(cols = c(1:10), names_to = "Subsite_Season", values_to = "log_est")
d2.se <- d2.se[,2]
